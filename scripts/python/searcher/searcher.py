...
class Searcher(QtWidgets.QWidget):
    ...
    def uisetup(self):
        ...
        phtext = (
            """<span class="right">Search..</span><span class="left">Begin typing or enter ':' to display options</span>""")
    ...
    def ctxsearcher(self, ctx=None):
        ...
        if ctx is None:
            self.ctxsearch = True
            print(self.tucctx)
            ctxresult = utility.PANETYPES[self.tucctx]
            print(ctxresult)
    ...
    def textchange_cb(self, text):
        if text in utility.CTXSHOTCUTS:
            self.ctxsearcher(text)
        elif len(text) > 1 and text not in utility.CTXSHOTCUTS:
            self.searching = True
    ...
    def openmenu(self):
        ...
        menu.setProperty("flat", True)
        menu.setStyleSheet(utility.MENUSTYLE)
        menu.setWindowFlags(menu.windowFlags() |
                            QtCore.Qt.NoDropShadowWindowHint)
        globalrefix = menu.addAction("Global items")
        contextprefix = menu.addAction("Context items")
        viewprefix = menu.addAction("View items")
        action = menu.exec_(self.searchbox.mapToGlobal(QtCore.QPoint(0, 20)))
        if action == globalrefix:
            self.searchbox.setText(":g")
        if action == contextprefix:
            self.searchbox.setText(":c")
        if action == viewprefix:
            self.searchbox.setText(":v")

        menu.setDefaultAction(globalrefix)
